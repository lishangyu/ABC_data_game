...
import keys
import numpy as np
import keras_train_cfg

characters = keys.alphabet[:]
from model import get_model

nclass = len(characters)+1 #分类数目 取决于词表个数+1

import keras.backend as K

trainroot = keras_train_cfg.trainroot
valroot   = keras_train_cfg.valroot
batchSize = 32
workers =  keras_train_cfg.workers
imgH = 32
imgW = 256
keep_ratio = False
random_sample = False
...
def one_hot(text,length=10,characters=characters):
     ...
     for i,char in enumerate(text.decode()):
        index = characters.find(str(char))
        if index==-1:
